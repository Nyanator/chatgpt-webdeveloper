...
import { MSG_KEY_BC, MSG_KEY_CB } from "../interfaces/content-peer-background";
...
(async () => {
    // コンテキスト間でのメッセージ通信を準備
    const messageAgent = await ChatGPTUtils.createRuntimeMessageAgent();
    ...
    reserveLoadedAction(document, () => {
        ...
        // コンテキスト間メッセージリスナーを設定
        messageAgent.addListener(async (message) => {
            if (message.key === MSG_KEY_BC.UrlUpdatedEvent) {
    ...
    function renderHtmlPreview(codeElement: HTMLElement) {
        ...
        const previewHtml = htmlTextToHtmlElement(
            codeElement.textContent ?? "",
        );

        // パフォーマンスのために描画の途中経過ではScriptを実行しません
        const allowScripts =
            codeBlockEnded === true || codeBlockStarted === false;
        if (oldPreviewWrapDiv && codeBlockEnded !== true) {
            ...
            if (
                oldPreviewFrame &&
                oldPreviewFrame.srcdoc === previewHtml.outerHTML
            ) {
                return;
            }
        }
    ...
    function checkMutationAndRserveObserve(mutationsList: MutationRecord[]) {
        const addNodes = mutationsList.flatMap((ml) =>
            Array.from(ml.addedNodes),
        );
        // ボディ内にボタンが存在する可能性があるため、HTML 要素のみをフィルタリング
        const htmlElements = addNodes
            .filter(
                (node) =>
                    node instanceof HTMLElement && node.nodeName !== "BUTTON",
            )
            .map((node) => node);
    ...
    async function showHTMLWindow(targetElement: HTMLElement) {
        ...
        // バックグラウンドに通知
        await messageAgent.sendMessage({
            message: {
                runtimeId: chrome.runtime.id,
                key: MSG_KEY_CB.ShowHTMLWindowRequest,
