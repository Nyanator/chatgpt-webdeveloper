import { findMatchParent, findParentPreElement, waitForAction } from "@nyanator/chrome-ext-utils";

import { LANGUAGE_PREFIX } from "./msg-def";
...
export const findInputForm = (): HTMLDivElement | null => {
  const formDiv = document.querySelector<HTMLDivElement>("div .absolute.bottom-0.left-0.w-full");
  return formDiv;
};
...
export const findNewChatButton = (): HTMLAnchorElement | undefined => {
  ...
  const newChatButton = buttons.find((button) => button.innerHTML.includes("New chat"));
 ...
 */
export const findCodeBlockDivFromCodeElement = (codeElement: HTMLElement): HTMLDivElement => {
  const codeBlockDiv = findMatchParent(codeElement, ".rounded-md.mb-4") as HTMLDivElement;
  return codeBlockDiv;
};
 ...
 * @param codeElement codeタグ要素
 */
export const findCodeElementFromClickElement = (event: MouseEvent): Element | null => {
  if (!(event.target instanceof HTMLElement) || !event.altKey) {
    return null;
  }

  const preElement = findParentPreElement(event.target);
  const codeElement = preElement?.querySelectorAll(`[class*="${LANGUAGE_PREFIX}"]`);
  if (!codeElement) {
    return null;
  }
